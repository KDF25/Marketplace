...
class FormMenuAdvertiser:
    ...
    async def main(self):
        ...
        text = text.substitute(main=self.__Text_lang.start.main,
                               text1=hlink(url=self.__Text_lang.url.advertiser.how_to_use,
                                           title=self.__Text_lang.information.how_to_use))
        return text

    async def how_to_use(self):
        text = Template('$text1')
        text = text.substitute(text1=hlink(url=self.__Text_lang.url.advertiser.how_to_use,
                                           title=self.__Text_lang.information.how_to_use))
        return text

    async def about_us(self):
        text = Template('$text1')
        text = text.substitute(text1=hlink(url=self.__Text_lang.url.advertiser.about_us,

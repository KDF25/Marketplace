...
class FormModerationGroup:
    ...
    async def _unpack_platform(self):
        ...
        # self.__type_platform = self.__data.get("type")
        ...
        self.__ban_reason = self.__data.get("ban_reason")
        self.__banned_at = self.__data.get("banned_at")
        await self._region()
    ...
    async def menu_unban(self):
        ...
        text = text.substitute(type_platform=self.__type_platform,
                               ...
                               region=self.__Lang.platform.blogger.form.region, region_platform=self.__region_platform,
                               symbol_platform=self.__symbol_platform,  price=self.__Lang.platform.blogger.form.price,
                               price_platform=self.__price_platform, date=self.__Lang.platform.blogger.form.date,
                               banned_at=self.__banned_at, reason=self.__Lang.platform.blogger.form.reason,
                               ban_reason=self.__ban_reason)
        return text

    async def menu_unban_platform(self):
        text = Template("<b>$unban</b>\n\n"
                        "<b>$platform:</b> $name\n\n"
                        "$myPlatform")
        text = text.substitute(unban=self.__Lang.group.moderation.user.unban,
                               name=hlink(url=self.__url, title=self.__name),
                               platform=self.__Lang.group.moderation.user.platform,
                               myPlatform=self.__Lang.group.moderation.user.myPlatform)
        return text

    async def menu_ban_platform(self):
        text = Template("$ban\n\n"
                        "<b>$platform:</b> $name\n\n"
                        "<b>$reason:</b> $text\n\n")
        text = text.substitute(ban=self.__Lang.group.moderation.user.ban,
                               name=hlink(url=self.__url, title=self.__name),
                               platform=self.__Lang.group.moderation.user.platform,
                               reason=self.__Lang.group.moderation.user.reason, text=self.__text)
        return text

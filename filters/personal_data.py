...
class IsLegalAddress(BoundFilter):
    async def check(self, message: types.Message):
        ...
        if len(message.text) <= 100:
            ...
            await bot.send_message(chat_id=message.from_user.id, text=Text_lang.alert.common.lengthLegalAddress)
    ...
    async def check(self, message: types.Message):
        ...
        if message.text.isnumeric() is True and len(message.text) <= 20:
            if int(message.text) >= Txt.commission.min_payment:
                ...
                await bot.send_message(chat_id=message.from_user.id, text=Text_lang.alert.common.minPayment)
        else:
            await bot.send_message(chat_id=message.from_user.id, text=Text_lang.alert.common.nonFormat)

...
class AddPlatformBlogger(StatesGroup):

    addPlatform_level1 = State()
    addPlatform_level2 = State()
    addPlatform_level3 = State()
    addPlatform_level4 = State()
    addPlatform_level5 = State()
    addPlatform_level6 = State()
    addPlatform_level7 = State()
    addPlatform_level8 = State()
    addPlatform_level9 = State()
    addPlatform_level10 = State()
    addPlatform_level11 = State()
    addPlatform_level12 = State()
    addPlatform_level13 = State()
    ...
    async def menu_kind(self, call: types.CallbackQuery, state: FSMContext):
        await self.addPlatform_level1.set()
    ...
    async def menu_platform(self, call: types.CallbackQuery, state: FSMContext):
        await self.addPlatform_level2.set()
        async with state.proxy() as data:
            reply, Lang, inline = await self._prepare(data)
    ...
    async def menu_title(self,  message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        async with state.proxy() as data:
            ...
            if isinstance(message, types.Message):
                ...
                await self.addPlatform_level3.set()
    ...
    async def menu_verification(self,  message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        message = message
        await self.addPlatform_level4.set()
        async with state.proxy() as data:
            reply, Lang, inline = await self._prepare(data)
    ...
    async def _category(self, message, Lang, inline):
        await self.addPlatform_level5.set()
        with suppress(MessageNotModified, MessageToEditNotFound):
    ...
    @staticmethod
    async def _category_turn(call, Lang, inline):
        with suppress(MessageNotModified, MessageToEditNotFound):
            await bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                        text=Lang.platform.blogger.category, reply_markup=await inline.menu_category())

    @staticmethod
    async def _callback_category(call, data):
    ...
    async def menu_lang(self, call: types.CallbackQuery, state: FSMContext):
        await self.addPlatform_level6.set()
        async with state.proxy() as data:
    ...
    async def _callback_lang(self, call, data):
        ...
        if dta[0] == "category":
            ...
            data.get("area")["category"]["value"] = \
                await self._get_value2(parameter_id=int(dta[1]), values=data.get("area").get("category").get("all_values"))
    ...
    async def _region(self, call, Lang, inline):
        await self.addPlatform_level7.set()
        with suppress(MessageNotModified, MessageToEditNotFound):
            await call.answer()
    ...
    async def _description(self, call, Lang, inline):
        await self.addPlatform_level8.set()
        with suppress(MessageNotModified, MessageToEditNotFound):
            await call.answer()
    ...
    async def menu_sex(self, message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        await self.addPlatform_level9.set()
        async with state.proxy() as data:
            reply, Lang, inline = await self._prepare(data)
            if isinstance(message, types.Message):
    ...
    async def menu_age(self, call: types.CallbackQuery, state: FSMContext):
        await self.addPlatform_level10.set()
        async with state.proxy() as data:
    ...
    async def _format(self, message, data):
        ...
        await self.addPlatform_level11.set()
        with suppress(MessageNotModified, MessageToEditNotFound):
            await message.answer()
    ...
    async def _symbol(self, call, data, Lang, inline):
        await self.addPlatform_level12.set()
        reply = ReplyUser(language=data.get('lang'))
    ...
    async def menu_check_platform(self, message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        await self.addPlatform_level13.set()
        async with state.proxy() as data:
    ...
    async def _end(call, data, Lang, reply):
        ...
        await bot.send_message(chat_id=call.from_user.id, text=Lang.platform.blogger.newAdd,
                               reply_markup=await reply.menu_blogger())

    def register_handlers_add_platform_blogger(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_kind, text="add",                                                  state="PlatformBlogger:platform_level1")
        dp.register_callback_query_handler(self.menu_kind, text="back",                                                 state=self.addPlatform_level2)

        dp.register_callback_query_handler(self.menu_platform, IsInstagram(), lambda x: x.data.startswith("kind"),      state=self.addPlatform_level1)

        dp.register_callback_query_handler(self.menu_platform, text="back", state=self.addPlatform_level3)

        dp.register_message_handler(self.menu_title, content_types='text', state=self.addPlatform_level2)
        dp.register_callback_query_handler(self.menu_title, text='back', state=self.addPlatform_level4)

        dp.register_message_handler(self.menu_verification, IsTitle(), content_types='text',
                                    state=self.addPlatform_level3)
        dp.register_callback_query_handler(self.menu_verification, text='back', state=self.addPlatform_level5)

        dp.register_callback_query_handler(self.menu_category, text='check',                                            state=self.addPlatform_level4)
        dp.register_callback_query_handler(self.menu_category, text='authorization',                                    state=self.addPlatform_level4)
        dp.register_callback_query_handler(self.menu_category_turn, text=['next', 'prev'],                              state=self.addPlatform_level5)
        dp.register_callback_query_handler(self.menu_category, text='back',                                             state=self.addPlatform_level6)

        dp.register_callback_query_handler(self.menu_lang, lambda x: x.data.startswith("category"),                     state=self.addPlatform_level5)
        dp.register_callback_query_handler(self.menu_lang, text='back',                                                 state=self.addPlatform_level7)
        dp.register_callback_query_handler(self.menu_change_lang, lambda x: x.data.startswith("platformLang"),          state=self.addPlatform_level6)

        dp.register_callback_query_handler(self.menu_region, text="confirm",                                            state=self.addPlatform_level6)
        dp.register_callback_query_handler(self.menu_region, text='back',                                               state=self.addPlatform_level8)
        dp.register_callback_query_handler(self.menu_change_region, lambda x: x.data.startswith("region"),              state=self.addPlatform_level7)

        dp.register_callback_query_handler(self.menu_description, text="confirm",                                       state=self.addPlatform_level7)
        dp.register_callback_query_handler(self.menu_description, text='back',                                          state=self.addPlatform_level9)

        dp.register_message_handler(self.menu_sex, IsDescription(), content_types='text',                               state=self.addPlatform_level8)
        dp.register_callback_query_handler(self.menu_sex, text='back',                                                  state=self.addPlatform_level10)

        dp.register_callback_query_handler(self.menu_age, lambda x: x.data.startswith("sex"),                           state=self.addPlatform_level9)
        dp.register_callback_query_handler(self.menu_age, text='back',                                                  state=self.addPlatform_level11)
        dp.register_callback_query_handler(self.menu_change_age, lambda x: x.data.startswith("age"),                    state=self.addPlatform_level10)

        dp.register_callback_query_handler(self.menu_format, text="confirm",                                            state=self.addPlatform_level10)
        dp.register_callback_query_handler(self.menu_format, text="back",                                               state=[self.price_level1, self.addPlatform_level12])
        dp.register_callback_query_handler(self.menu_format, text="clean",                                              state=self.addPlatform_level11)
        dp.register_message_handler(self.menu_format, IsPrice(), content_types='text',                                  state=self.price_level1)

        dp.register_callback_query_handler(self.menu_price, lambda x: x.data.startswith("format"),                      state=self.addPlatform_level11)

        dp.register_callback_query_handler(self.menu_symbol, text="confirm",                                            state=self.addPlatform_level11)
        dp.register_callback_query_handler(self.menu_symbol, text="back",                                               state=self.addPlatform_level13)

        dp.register_message_handler(self.menu_check_platform, text=Txt.common.skip,                                     state=self.addPlatform_level12)
        dp.register_message_handler(self.menu_check_platform, LenSymbol(), content_types='text',                        state=self.addPlatform_level12)

        dp.register_callback_query_handler(self.menu_end, text="alright",                                               state=self.addPlatform_level13)

...
class ExistBlogger(StatesGroup):

    existBlogger_level1 = State()
    existBlogger_level2 = State()
    existBlogger_level3 = State()
    existBlogger_level4 = State()
    ...
    async def menu_have_account(self, call: types.CallbackQuery, state: FSMContext):
        await self.existBlogger_level1.set()
    ...
    async def menu_forgot_password(self, message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        await self.existBlogger_level2.set()
        async with state.proxy() as data:
            ...
            message1 = await bot.send_message(chat_id=message.from_user.id,
                                              text=Lang.registration.existAccount.password,
                                              reply_markup=await inline.menu_forgot_password())
            data['message_id'] = message1.message_id

    async def menu_code(self, call: types.CallbackQuery, state: FSMContext):
        await self.existBlogger_level3.set()
        async with state.proxy() as data:
    ...
    async def _success_code(self, message, data, Lang, inline):
        await self.existBlogger_level4.set()
        with suppress(MessageToDeleteNotFound, MessageIdentifierNotSpecified, MessageCantBeDeleted):
            await bot.delete_message(chat_id=message.from_user.id, message_id=data.get('message_id'))
    ...
    async def _main_menu(message, data, reply, Lang):
        with suppress(MessageToDeleteNotFound, MessageIdentifierNotSpecified, MessageCantBeDeleted):
            await bot.delete_message(chat_id=message.from_user.id, message_id=data.get('message_id'))
        await bot.send_message(chat_id=message.from_user.id, reply_markup=await reply.menu_blogger(),
                               text=Lang.start.greeting)
    ...
    def register_handlers_exist_blogger(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_have_account, HaveAccount(), text="haveAccount",                   state="RegistrationBlogger:regBlogger_level2")
        dp.register_callback_query_handler(self.menu_have_account, text="back",                                         state=self.existBlogger_level2)

        dp.register_message_handler(self.menu_forgot_password,  IsEmail(), IsExist(), content_types='text',             state=self.existBlogger_level1)
        dp.register_callback_query_handler(self.menu_forgot_password, text='back',                                      state=self.existBlogger_level3)

        dp.register_callback_query_handler(self.menu_code, text='forgotPassword',                                       state=self.existBlogger_level2)
        dp.register_callback_query_handler(self.menu_code, text='back',                                                 state=self.existBlogger_level4)

        dp.register_callback_query_handler(self.menu_code, text='resendCode',                                           state=self.existBlogger_level3)

        dp.register_message_handler(self.menu_password, IsCode(), content_types='text',                                 state=self.existBlogger_level3)

        dp.register_message_handler(self.menu_main, content_types='text',                                               state=self.existBlogger_level2)
        dp.register_message_handler(self.menu_update_password, IsPassword(), content_types='text',                      state=self.existBlogger_level4)

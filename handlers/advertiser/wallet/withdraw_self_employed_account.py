...
class WithdrawSelfEmployedAccountAdvertiser(StatesGroup):
    ...
    async def _check_withdraw(self, message, data):
        ...
        if int(message.text)  > data.get("wallet"):
            ...
            data.get("selfEmployedAccount")["cash"] = int(message.text)
    ...
    async def _end(call, data, form_wallet, reply):
        with suppress(MessageToDeleteNotFound, MessageIdentifierNotSpecified, MessageCantBeDeleted):
            await bot.delete_message(chat_id=call.from_user.id, message_id=data.get("message_id"))
        await bot.send_message(chat_id=call.from_user.id, text=await form_wallet.menu_end(type_legal="selfEmployedAccount"),
    ...
    async def _prepare_end(data):
        ...
        form_wallet = FormWallet(cash=data.get('selfEmployedAccount').get("cash"), language=data.get('lang'))
        return Lang, reply, form_wallet

    @staticmethod
    async def _send_group(data):
        send_group = SendWithdraw(data=data, type_legal="selfEmployedAccount")
        await send_group.start()

    def register_handlers(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_personal_data, text="selfEmployedAccount",                         state="WalletAdvertiser:withdraw_level1")

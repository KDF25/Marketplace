...
class PaymentIndividualAdvertiser(StatesGroup):
    ...
    async def _change_edit(message: Union[types.Message, types.CallbackQuery], inline, form):
        with suppress(MessageNotModified, MessageToEditNotFound):
            ...
            await bot.edit_message_text(chat_id=message.from_user.id, text=await form.menu_change_data(),
                                        message_id=message.message.message_id, reply_markup=await inline.menu_change_individual())
    ...
    async def _payment_didox(data):
        json = WalletModel(type_legal="individual", amount=data.get("individual").get("cash"))
        await fastapi.payment_didox(json=json, token=data.get('token'))

    def register_handlers(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_personal_data, text="individual",                                  state="PaymentCommonAdvertiser:paymentCommon_level2")

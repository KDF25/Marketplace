...
class WalletAdvertiser(StatesGroup):
    ...
    async def _wallet(message, form, inline, Lang, reply, data):
        await bot.send_message(chat_id=message.from_user.id, text=Lang.menu.advertiser.wallet,
    ...
    def register_handlers(self, dp: Dispatcher):
        dp.register_message_handler(self.menu_wallet, text=Txt.menu.wallet,                                              state="MenuAdvertiser:menuAdvertiser_level1")
        dp.register_callback_query_handler(self.menu_wallet, text="back",                                               state=["PaymentCommonAdvertiser:paymentCommon_level1",
                                                                                                                               self.withdraw_level1])
        dp.register_callback_query_handler(self.menu_withdraw_start, text="withdraw",                                   state=self.walletAdvertiser_level1)
        dp.register_callback_query_handler(self.menu_withdraw_start, text="back",                                       state=["WithdrawEntityAdvertiser:withdraw_level1",
                                                                                                                               "WithdrawIndividualAdvertiser:withdraw_level1",
                                                                                                                               "WithdrawSelfEmployedAccountAdvertiser:withdraw_level1",
                                                                                                                               "WithdrawSelfEmployedCardAdvertiser:withdraw_level1"])

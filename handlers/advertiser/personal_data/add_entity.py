...
class AddDataEntityAdvertiser(StatesGroup):
    ...
    async def _callback_data(data):
        if data.get('new_entity') is None:
            data["new_entity"] = {}
    ...
    async def _prepare(data):
        ...
        form = FormEntityData(data=data.get("new_entity"), language=data.get('lang'), email=data.get("email"))
    ...
    async def menu_get_title(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["title"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_legal_address(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["legalAddress"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_inn(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["inn"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_payment_account(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["paymentAccount"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_bank(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["bank"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_mfo(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            print(data)
            data.get('new_entity')["mfo"] = message.text
            await self._change_data(message, data)
    ...
    async def menu_get_phone(self, message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            ...
            data.get('new_entity')["phone"] = phone
            await self._change_data(message, data)
    ...
    async def _add_entity(data):
        json = await func.add_entity(data=data.get("new_entity"))
        await fastapi.add_type_legal(json=json, token=data.get('token'))

    def register_handlers_personal(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_add_data, text="entity",                                           state="PersonalDataAdvertiser:personalDataAdvertiser_level2")
        ...
        dp.register_callback_query_handler(self.menu_inn, text="inn",                                                   state=self.addData_level2)

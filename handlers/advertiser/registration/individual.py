...
class FirstIndividualAdvertiser(StatesGroup):
    ...
    async def menu_get_phone(self,  message: types.Message,  state: FSMContext):
        async with state.proxy() as data:
            data.get("individual")["phone"] = message.text
            await self._change_data(message, data)
    ...
    def register_handlers(self, dp: Dispatcher):
        dp.register_callback_query_handler(self.menu_first_data, text="individual",                                     state="RegistrationAdvertiser:regAdvertiser_level6")

...
class RegistrationAdvertiser(StatesGroup):

    regAdvertiser_level0 = State()
    regAdvertiser_level1 = State()
    regAdvertiser_level2 = State()
    regAdvertiser_level3 = State()
    regAdvertiser_level4 = State()
    regAdvertiser_level5 = State()
    regAdvertiser_level6 = State()

    async def menu_agreement(self,  message: types.Message, state: FSMContext):
        await self.regAdvertiser_level1.set()
        async with state.proxy() as data:
            ...
            await bot.send_message(chat_id=message.from_user.id, text=Lang.start.advertiser,
    ...
    async def menu_login(self, call: types.CallbackQuery, state: FSMContext):
        await self.regAdvertiser_level2.set()
        async with state.proxy() as data:
            Lang, inline = await self._prepare(data)
    ...
    async def menu_code(self, message: types.Message, state: FSMContext):
        await self.regAdvertiser_level3.set()
        async with state.proxy() as data:
    ...
    async def _success_code(self, message, data, Lang, inline):
        await self.regAdvertiser_level4.set()
        with suppress(MessageToDeleteNotFound, MessageIdentifierNotSpecified, MessageCantBeDeleted):
    ...
    async def menu_registration(self, message: types.Message, state: FSMContext):
        ...
        async with state.proxy() as data:
            ...
            await self._rec_advertiser(message, data)
    ...
    async def _prepare_rec(data):
        ...
        return Lang, inline_rec, reply, form_platform

    async def _rec_advertiser(self, message, data):
        user = User(email=data.get('email'), password_hash=data.get("password"), role="advertiser", wallet=0,
    ...
    async def _check_status_rec(self, message, data, status, json):
        Lang, inline_rec, reply, form_platform = await self._prepare_rec(data)
        if status == 201:
            await self._get_token(data)
            await self._success_rec(message, data, json, Lang, inline_rec, reply, form_platform)
        else:
            await bot.send_message(chat_id=message.from_user.id, text=Lang.alert.common.error)

    async def _success_rec(self, message, data, json, Lang, inline_rec, reply, form_platform):
        await self.regAdvertiser_level5.set()
        await pg.update_telegram_user(user_id=message.from_user.id, email=data.get('email'), client_id=json.get("id"))
        await self._end(message, data, Lang, inline_rec, reply, form_platform)
    ...
    async def _end(self, message, data, Lang, inline_rec, reply, form_platform):
        message2 = await bot.send_message(chat_id=message.from_user.id, text=".",
                                          reply_markup=await reply.menu_next_time())
        await self._delete_message(message, data)
        message1 = await bot.send_message(chat_id=message.from_user.id, text=await form_platform.menu_start(),
                                          reply_markup=await inline_rec.menu_add_data())
    ...
    async def menu_add_data(self, message: Union[types.Message, types.CallbackQuery], state: FSMContext):
        await self.regAdvertiser_level6.set()
        async with state.proxy() as data:
            Lang, inline_rec, reply, form_platform = await self._prepare_rec(data)
            await self._add_data(message, inline_rec, form_platform)

    @staticmethod
    async def _add_data(call, inline_rec, form_platform):
        with suppress(MessageNotModified, MessageToEditNotFound):
            await call.answer()
            await bot.edit_message_text(chat_id=call.from_user.id, message_id=call.message.message_id,
                                        text=await form_platform.menu_personal_data(),
                                        reply_markup=await inline_rec.menu_employment())

    def register_handlers_registration_advertiser(self, dp: Dispatcher):
        dp.register_message_handler(self.menu_agreement, text=Txt.start.start,                                          state=self.regAdvertiser_level0)

        dp.register_callback_query_handler(self.menu_start, text="back",                                                state=self.regAdvertiser_level2)

        dp.register_callback_query_handler(self.menu_login, text="agree",                                               state=self.regAdvertiser_level1)
        dp.register_callback_query_handler(self.menu_login, text="back",                                                state=[self.regAdvertiser_level3, self.regAdvertiser_level4, "ExistAdvertiser:existAdvertiser_level1"])

        dp.register_message_handler(self.menu_code, IsNew(), IsEmail(), content_types='text',                           state=self.regAdvertiser_level2)

        dp.register_callback_query_handler(self.menu_resend_code, text='resendCode',                                    state=self.regAdvertiser_level3)
        dp.register_message_handler(self.menu_password, IsCode(), content_types='text',                                 state=self.regAdvertiser_level3)
        dp.register_message_handler(self.menu_registration, IsPassword(), content_types='text',                         state=self.regAdvertiser_level4)
